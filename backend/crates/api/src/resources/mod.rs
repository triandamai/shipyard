...
async fn sync_traefik_dynamic_config(
    ...
    // Resolve slug for file name (falls back to UUID string)
    let slug: String = sqlx::query_scalar("SELECT slug FROM services WHERE id = $1")
        .bind(service_id)
        .fetch_optional(db)
        .await
        .ok()
    ...
    let docker_svc_name = format!("{label_prefix}-{service_id}");
