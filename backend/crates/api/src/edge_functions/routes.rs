...
/// Write a per-domain Traefik config file: `edge-domain-{domain_id}.yml`.
/// One file per domain — adding or removing a domain only touches that file.
async fn write_domain_traefik_config(
    domain_id: Uuid,
    hostname: &str,
    tls_enabled: bool,
    cert_provider: &str,
    ...
    let path = std::path::Path::new(dir).join(format!("edge-domain-{domain_id}.yml"));

    let mut out = String::from("# Auto-generated by Shipyard — do not edit\n");
    out.push_str("http:\n");
    ...
    if let Err(e) = tokio::fs::create_dir_all(dir).await {
        tracing::warn!("edge traefik: could not create dir '{dir}': {e}");
        return;
    }
    if let Err(e) = tokio::fs::write(&path, out.as_bytes()).await {
        tracing::warn!("edge traefik: could not write '{path:?}': {e}");
    } else {
        tracing::info!("Wrote edge Traefik config: {path:?}");
    }
}

/// Remove the per-domain Traefik config file for `domain_id`.
async fn remove_domain_traefik_config(domain_id: Uuid, dynamic_config_dir: Option<&str>) {
    let Some(dir) = dynamic_config_dir else { return };
    let path = std::path::Path::new(dir).join(format!("edge-domain-{domain_id}.yml"));
    if let Err(e) = tokio::fs::remove_file(&path).await {
        tracing::warn!("edge traefik: could not remove '{path:?}': {e}");
    } else {
        tracing::info!("Removed edge Traefik config: {path:?}");
    }
}
...
async fn create_group_domain(
    ...
    write_domain_traefik_config(
        domain.id,
        &domain.hostname,
        domain.tls_enabled,
        &domain.cert_provider,
        domain.port,
        org_id,
...
async fn delete_group_domain(
    ...
    remove_domain_traefik_config(domain_id, state.config.traefik.dynamic_config_dir.as_deref()).await;
    publish_topology_changed(&state, org_id, group_id).await;
    Ok(Json(ApiResponse::ok(serde_json::json!({ "deleted": true }))))
}
