...
fn edge_upstream(org_id: Uuid) -> String {
    format!("shipyard-edge-{}", &org_id.to_string()[..8])
}
...
async fn delete_group_domain(
    ...
    // Resolve service_id before deleting — needed for post-delete Traefik sync.
    let service_id: Option<Uuid> = sqlx::query_scalar(
        "SELECT service_id FROM edge_function_groups WHERE id = $1 AND org_id = $2",
    )
    .bind(group_id)
    ...
    let service_id = service_id
        .ok_or_else(|| ApiAppError(AppError::NotFound("group not found".into())))?;
