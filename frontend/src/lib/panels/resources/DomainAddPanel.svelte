<script lang="ts">
	...
	import { Globe, Shield, ShieldOff, Dice5, Lock, Unlock } from '@lucide/svelte';
	...
	let { serviceId, onCreated }: Props = $props();
	...
	const ADJECTIVES = [
		...
		'dusty','empty','fresh','grand','hollow','ivory','jade','knotty','lunar',
		'misty','noble','olive','polar','rosy','sunny','tangy','ultra','velvet',
	];
	const NOUNS = [
	...
	// cert_provider sent to API: use customCert when certProvider === 'custom'
	...
	async function handleSubmit(e: SubmitEvent) {
		...
		const parsedPort = portRaw ? parseInt(portRaw, 10) : null;
			const res = await api.createDomain(serviceId, {
...
<div class="panel-wrap">
	<form class="form" onsubmit={handleSubmit}>
		...
		<div class="form-group">
			<label class="form-label" for="da-hostname">Hostname</label>
			<div class="hostname-row">
				...
				<button type="button" class="dice-btn" onclick={roll} title="Generate random Traefik subdomain">
					<Dice5 size={15} />
				</button>
			</div>
		...
		{#if tlsEnabled}
			<div class="form-group">
				<label class="form-label" for="da-cert">Certificate Provider</label>
		...
		<!-- Port mapping -->
		<div class="form-group">
			<label class="form-label" for="da-port">Container Port</label>
			<div class="input-icon-wrap">
				{#if tlsEnabled}
					<Lock size={13} class="input-icon" />
				{:else}
					<Unlock size={13} class="input-icon" />
				{/if}
				<input
					id="da-port"
					class="form-input with-icon font-mono"
			...
			<span class="form-hint">
				The port your container listens on. Traefik will route traffic here.
				Leave blank to use the first exposed port.
			</span>
		</div>
		...
		{#if hostname.trim()}
			<div class="preview-card">
				...
				<code class="preview-route">
					{tlsEnabled ? 'https' : 'http'}://{hostname.trim()}{String(portStr ?? '').trim() ? ` → :${String(portStr ?? '').trim()}` : ''}
				</code>
				{#if tlsEnabled}
...
<style>
	...
	.form-input.with-icon {
		background: transparent; border: none; padding-left: 32px; flex: 1;
	}
	...
	.tls-toggle:hover { border-color: var(--border-hover, var(--border)); }
	.tls-toggle.tls-on {
