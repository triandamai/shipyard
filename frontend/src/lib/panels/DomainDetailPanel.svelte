<script lang="ts">
	...
	function openExternal() {
		...
		const portSuffix = domain.port && domain.port !== (domain.tls_enabled ? 443 : 80)
			? `:${domain.port}` : '';
		window.open(`${protocol}://${domain.hostname}${portSuffix}`, '_blank', 'noopener');
	}
</script>
...
<div class="panel-body">
	...
	{:else if domain}
				...
				<div class="hero-info">
					...
					<div class="hero-badges">
						{#if domain.tls_enabled}
							<span class="badge badge-green"><Shield size={10} /> TLS enabled</span>
						{:else}
							<span class="badge badge-muted"><ShieldOff size={10} /> No TLS</span>
						{/if}
						{#if domain.port}
		...
		<section class="section">
			...
			{:else if dnsStatus === 'fail'}
				<div class="dns-result dns-fail">
					<X size={13} />
					<span>{dnsResult ? `Does not resolve (${domain.hostname})` : 'Check failed'}</span>
				</div>
			{:else if dnsStatus === 'idle'}
				<p class="hint">Run a DNS check to verify {domain.hostname} points to this server.</p>
			{/if}
		</section>
		...
		<section class="section danger-section">
			...
			{#if !showDelete}
				<button class="btn btn-danger btn-sm" onclick={() => (showDelete = true)}>
			...
			{:else}
				...
				<div class="confirm-actions">
					<button class="btn btn-secondary btn-sm" onclick={() => { showDelete = false; deleteInput = ''; }}>
						Cancel
					</button>
					<button
...
<style>
	...
	.dns-ok   { background: var(--accent-green-muted); color: var(--accent-green); }
	.dns-fail { background: color-mix(in srgb, var(--accent-red) 10%, transparent); color: var(--accent-red); }
	...
	.confirm-hint strong { color: var(--text-primary); }
