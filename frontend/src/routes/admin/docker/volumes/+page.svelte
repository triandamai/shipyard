<script lang="ts">
	...
	async function prune() {
		if (!confirm('Prune unused volumes? This cannot be undone.')) return;
	...
	let filtered = $derived(
		volumes.filter(v => !search || v.name.toLowerCase().includes(search.toLowerCase()))
	);
...
<div class="inner-toolbar">
	...
	<div class="right-controls">
		<label class="search">
			...
			<input type="text" placeholder="Search volumes…" bind:value={search} />
		</label>
...
{:else if paged.length === 0}
	<div class="empty">No volumes found.</div>
{:else}
	<div class="tbl">
		<div class="thead">
		...
		{#each paged as v}
			...
			<div class="trow-wrapper">
				<div class="trow" onclick={() => toggleExpand(v.name)}>
					<div class="mono trunc expand-cell" style="flex:2;font-size:12px">
						<span class="chevron" class:expanded={expanded.has(v.name)}>▶</span>
						{v.name}
					</div>
					<div class="cell" style="flex:1">{v.driver}</div>
					<div class="cell" style="flex:1">{v.scope}</div>
					<div class="mono muted trunc" style="flex:3;font-size:11px">{v.mountpoint}</div>
				</div>
				{#if expanded.has(v.name)}
					<div class="details-panel">
						<div class="details-grid">
							<div style="grid-column: 1 / -1"><strong>Mountpoint:</strong> <span class="mono">{v.mountpoint}</span></div>
