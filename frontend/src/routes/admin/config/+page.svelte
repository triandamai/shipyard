...
<div class="p">
	<div class="maint-banner" class:maint-on={maintMode}>
		<div class="maint-l">
			...
			<div>
				...
				<div class="maint-desc">{maintMode ? 'Platform is currently in maintenance mode — users see a maintenance page.' : 'Platform is operating normally.'}</div>
			</div>
		</div>
		<button class="maint-btn" class:maint-btn-on={maintMode} onclick={toggleMaintenance} disabled={savingMaint}>
			{savingMaint ? '…' : maintMode ? 'Disable Maintenance' : 'Enable Maintenance'}
		</button>
	</div>
...
<style>
	...
	.maint-banner { display:flex; align-items:center; justify-content:space-between; gap:12px; padding:14px 18px; margin-bottom:24px; border-radius:var(--radius); border:1px solid var(--border); background:var(--surface); box-shadow:var(--shadow-sm); flex-wrap:wrap; }
	.maint-banner.maint-on { background:var(--warn-soft); border-color:rgba(180,83,9,0.25); }
	.maint-l { display:flex; align-items:flex-start; gap:10px; color:var(--text-3); }
	.maint-banner.maint-on .maint-l { color:var(--warn); }
	.maint-title { font-size:13px; font-weight:700; color:var(--text); margin-bottom:2px; }
	.maint-banner.maint-on .maint-title { color:var(--warn); }
	.maint-desc { font-size:11.5px; color:var(--text-3); }
	.maint-btn { padding:6px 14px; height:32px; border-radius:var(--radius-sm); font-size:12px; font-weight:600; cursor:pointer; border:1px solid var(--border); background:var(--surface-2); color:var(--text-2); white-space:nowrap; font-family:var(--font); transition:background .15s; flex-shrink:0; }
	.maint-btn:hover:not(:disabled) { background:var(--accent); border-color:var(--accent); color:#000; }
	.maint-btn.maint-btn-on { background:var(--warn-soft); color:var(--warn); border-color:rgba(180,83,9,0.3); }
	.maint-btn.maint-btn-on:hover:not(:disabled) { background:var(--warn); color:#fff; }
	.maint-btn:disabled { opacity:.5; cursor:not-allowed; }
