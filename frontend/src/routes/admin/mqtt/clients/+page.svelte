<script lang="ts">
	...
	let filtered = $derived(
		clients.filter(c => {
			...
			const ip = (c.remote_addr ?? c.ip_address ?? c.ipaddress ?? '').toLowerCase();
			return !q || cid.includes(q) || user.includes(q) || ip.includes(q);
		})
	);
...
<div class="search-wrap">
	...
	<input class="search-inp" placeholder="Filter clients by ID, username, IP…" bind:value={search} />
</div>
...
{:else}
	<div class="tbl">
		...
		{#each paged as c}
			<div class="trow">
				...
				<div class="mono" style="flex:1.5;font-size:11.5px">{c.remote_addr ?? c.ip_address ?? c.ipaddress ?? '—'}</div>
	...
	<div class="card-list">
		{#each paged as c}
			<div class="m-card">
				<div class="m-card-title mono">{c.client_id ?? c.clientid ?? '—'}</div>
				<div class="m-card-row"><span class="m-key">Username</span><span>{c.username ?? '—'}</span></div>
				<div class="m-card-row"><span class="m-key">Address</span><span class="mono">{c.remote_addr ?? c.ip_address ?? c.ipaddress ?? '—'}</span></div>
				<div class="m-card-row"><span class="m-key">Protocol</span><span>{c.proto_ver ?? c.protocol ?? '—'}</span></div>
				<div class="m-card-row"><span class="m-key">Connected</span><span>{connectedAt(c.connected_at ?? c.connected_epoch)}</span></div>
			</div>
		{/each}
	</div>
